...
"[project]/app/api/admin/delete-product/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    ...
    try {
        ...
        const { data: existingProduct, error: checkError } = await supabase.from("products").select("*").eq("id", productId).single();
        ...
        const { error: ordersDeleteError } = await adminClient.from("orders").delete().eq("product_id", productId).eq("status", "pending") // Only delete pending orders
        ;
        ...
        const { data: deletedData, error: productDeleteError } = await adminClient.from("products").delete().eq("id", productId).eq("seller_id", sellerId) // Extra security check
...
//# sourceMappingURL=%5Broot-of-the-server%5D__944000cf._.js.map